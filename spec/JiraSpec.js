...
describe("Jira Controller" , function() {
	...
	it("projects in have total cost", function(){
		//loadTaffyWithMockData(mockJiraResponse);
		getProjectlogs().forEach(function(log){
			expect(log.cost).toBeGreaterThan(0);
		});
	});

	it("first project in mock has total cost of 1275", function(){
		//loadTaffyWithMockData(mockJiraResponse);
		var logs = getProjectlogs();
		expect(logs[0].cost).toBe(1275);
	});
